...
#
# Usage:
#   ./scripts/build-rust-ohos.ps1                          # both archs, auto-detect SDK
#   ./scripts/build-rust-ohos.ps1 -Archs aarch64           # single arch
#   ./scripts/build-rust-ohos.ps1 -SdkRoot <SDK root>      # SDK dir containing native/llvm/bin/clang.exe
#   ./scripts/build-rust-ohos.ps1 -RustDir <rust crate>    # xianyu_core crate dir (default: main project)
#
# Artifacts:
...
$PocDir    = Split-Path -Parent $ScriptDir                     # poc project root

# Rust crate dir resolution: param > env > legacy relative layout > known main-project path
if (-not $RustDir -and $env:XIANYU_RUST_DIR) { $RustDir = $env:XIANYU_RUST_DIR }
if (-not $RustDir) {
    $legacy = Join-Path (Split-Path -Parent $PocDir) 'rust'
    if (Test-Path (Join-Path $legacy 'Cargo.toml')) { $RustDir = $legacy }
}
if (-not $RustDir) { $RustDir = 'D:\Program Files\XianYu-Music\XianYu-Music-Mobile\rust' }
...
foreach ($arch in $Archs) {
    ...
    $DestDir = Join-Path $PocDir "ohos\entry\libs\$($m.Libs)"
    if (Test-Path (Join-Path $PocDir 'ohos\entry')) {
        ...
        Write-Warning 'ohos/entry missing (run setup first), artifact kept under rust/target'
    }
}
