...
//  4. QuickJS 插件引擎运行时（pluginEngineInit + pluginInstallScript 试运行）
...
class _PocPageState extends State<PocPage> {
  ...
  void initState() {
    ...
    _cases = [
      ...
      _PocCase('4. QuickJS 插件引擎', '安装最小 MusicFree 格式脚本（内部 QuickJS 试运行）', () async {
        ...
        final r = await rust.pluginInstallScript(
          dataDir: dataDir,
          script: script,
          origin: 'poc://inline',
        );
      ...
      _PocCase('5. SQLite（rusqlite bundled）', 'C 交叉编译组件 + 读写往返验证', () async {
        ...
        await rust.statsRecordPlay(
          dbPath: db,
          payloadJson:
              '{"songPath":"poc://a.mp3","listenedMs":5000,"durationMs":200000,"title":"PoC","artist":"PoC"}',
        );
