...
void main() {
  ...
  // 持续涟漪已移除（BiliPai 透镜是静态的，无时间驱动波）：此前的 uTime
  // 行进波会把按住/拖动时折叠进边带的内容（图标/文字）撕成随时间晃动的
  // 碎片，且与静止冻结抓屏（blit 无动画）互相矛盾。
  // 槽 19 uTime 保留占位（Impeller 剔除未引用 uniform 会导致槽位错位）。
  uv -= vec2(0.0, uTime * 0.0);
