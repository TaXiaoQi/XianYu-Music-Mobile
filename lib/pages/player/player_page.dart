    ...
    return Row(
      children: [
        ...
        IconButton(
          ...
          onLongPress: () => _showShareSettings(context, ref),
        ),
        if (current.isOnline)
          IconButton(
...
/// 打开分享设置弹窗（长按分享按钮触发）：分享链接有效时长 + 分享链接播放失败行为。
void _showShareSettings(BuildContext context, WidgetRef ref) {
  showSheetDialog<void>(context, (_) => const _ShareSettingsSheet());
}

/// 分享设置：长按播放页分享按钮弹出，配置分享链接有效时长与播放失败行为。
class _ShareSettingsSheet extends ConsumerWidget {
  const _ShareSettingsSheet();

  /// 有效时长预设（分钟）：覆盖 5 分钟 ~ 24 小时。
  static const _validityPresets = <(int, String)>[
    (5, '5 分钟'),
    (30, '30 分钟'),
    (60, '1 小时'),
    (120, '2 小时'),
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    final scheme = Theme.of(context).colorScheme;
    final settings = ref.watch(settingsProvider).valueOrNull;
    final minutes = settings?.shareLinkValidityMinutes ?? 120;
    final behavior = settings?.sharePlaybackFailureBehavior ?? 'pause';
      ...
      child: Column(
        ...
        children: [
          ...
          Wrap(
            ...
            runSpacing: 8,
            children: [
              for (final (val, label) in _validityPresets)
                _PresetChip(
                  label: label,
                  selected: minutes == val,
                  onTap: () => ref
                      .read(settingsProvider.notifier)
                      .setShareLinkValidityMinutes(val),
                ),
            ],
          ),
          ...
          _BehaviorTile(
            title: '暂停播放',
            subtitle: '分享歌曲起播失败时停止并显示错误',
            selected: behavior == 'pause',
            onTap: () => ref
                .read(settingsProvider.notifier)
                .setSharePlaybackFailureBehavior('pause'),
          ),
          _BehaviorTile(
            title: '替换播放',
            subtitle: '按来源信息走插件索引换源重播同一首歌',
            selected: behavior == 'replace',
            onTap: () => ref
                .read(settingsProvider.notifier)
                .setSharePlaybackFailureBehavior('replace'),
          ),
