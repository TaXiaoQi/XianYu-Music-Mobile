...
- 🎨 **高颜值液态玻璃 UI**
  ...
  - **壁纸中心**：壁纸广场云端壁纸库下载，登录后可上传分享并多端同步；自定义壁纸支持静态图片 / 视频动态壁纸 / 动态照片（Motion Photo）内嵌视频自动提取。

- 🎧 **专业音频引擎**

  - **全格式解码**：基于 `symphonia`，支持 MP3 / FLAC / AAC / ALAC / OGG / Vorbis / WAV / AIFF，另支持 DSD（DSF）以 DoP 方式播放。
  - **倍速播放**：0.5~2.0 倍速变速不变调，播客 / 有声内容友好。
  - **QMC2 解密**：内置加密格式解密（兼容 QMC1 旧格式、ekey 自动提取），插件源加密资源直接播放。
...
- 🎬 **MV 与在线内容**
  ...
  - **MV 播放**：自动探测歌曲 MV 入口，视频硬解播放；音画频谱对齐自动校准起播偏移（全局对齐 + 局部滑窗兜底），探测结果缓存避免重复请求。
  - **内置五源搜索**：酷我 / 网易云 / 酷狗 / QQ 音乐 / 咪咕内置聚合搜索，不装插件也能搜在线歌曲。
  - **音源榜单**：自动检测支持排行榜的插件音源，动态加载各源官方榜单，首页榜单速览。
  - **歌曲评论**：播放页评论区，插件评论拉取 + 平台评论兜底，支持查看回复。
...
- 📱 **平台原生体验**
  ...
  - **桌面小组件**：Android 桌面播放小组件；iOS 小组件实时同步歌词 / 封面 / 播放状态。
  - **应用内反馈**：登录后一键提交，附带错误日志 / 全量日志 / 截图。
  - **应用内自更新**：检查新版本并引导下载安装，商店渠道（Play / F-Droid）自动跳过提示。
  - **本地音乐库**：`rayon` 并行扫描、标签解析、封面提取与调色板、增量差异更新。
  - **听歌识曲**：识别正在播放的外部歌曲并直达播放，支持桌面识曲小组件入口。
  - **扫码授权登录**：扫桌面版登录二维码，确认设备信息后一键授权，免手动输密码。
  - **多语言**：简体中文 / 繁体中文 / English 界面语言切换。

- ⌚ **腕上联动**

  - **手表直连**：蓝牙 RFCOMM 直连「腕上弦予」，播放遥控 / 归一化歌词推送 / 进度插值全走自研 XYW1 协议。
  - **华为 Wear Engine**：华为手机免开 App 静默唤起腕上端，开播即达表盘；非华为设备自动降级为纯蓝牙直连。
  - **云端通道兜底**：蓝牙直连之外再经云端 WebSocket 通道中转，直连不可达时遥控与歌词推送不断联。

- 🌐 **远程与投放**

  - **双格式插件**：兼容 MusicFree / LX 落雪插件，QuickJS 沙箱执行，HTTP 请求经 Rust 代理无 CORS 限制；支持多订阅源管理与插件更新检查。
  ...
  - **外部歌单导入**：支持 BakaMusic / MusicFree / 洛雪备份（JSON / ZIP / lxmc）、M3U / M3U8 播放列表、椒盐音乐 TXT 与本地文件夹导入；云端歌单一键导入并关联插件源，可从源端拉取更新对比增删。
  - **WebDAV 远程音源**：远程曲库扫描、LRU 缓存、流式播放。
  - **DLNA 投屏**：SSDP 自动发现局域网渲染设备，一键投放到电视 / 音箱。
  - **下载与工具箱**：歌曲下载（音质 / 路径 / 并发 / 标签嵌入可配），音频格式转换与剪辑（原格式 / 转码输出）、QMC 解密工具、批量重命名；插件加密源解密落盘。
  - **分享与 Deep Link**：外部歌曲 / 歌单链接直接唤起播放，支持「下一首播放」插入；歌曲可生成分享链接，一键分享到 QQ 或系统分享面板。
  - **歌词**：QRC / LYS / YRC 逐字歌词，AMLL 风格渲染，本地缓存 + 远程获取；悬浮歌词（字体 / 颜色 / 位置 / 翻译 / 罗马音可配）、状态栏歌词、自定义歌词字体（ttf / otf 导入），点击歌词行跳转进度。

- 🧭 **个性化与统计**

  - **每日推荐**：本地化推荐算法，按听歌偏好生成每日歌单。
  - **最近播放**：播放历史自动记录，独立页面随时回溯重播。
  - **热搜榜**：服务端实时热搜词，热门搜索一屏可见；搜索历史自动记录（最多 10 条）。
