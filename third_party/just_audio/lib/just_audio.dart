...
_ProxyHandler _proxyHandlerForUri(
  ...
  Future<void> handler(_ProxyHttpServer server, HttpRequest request) async {
    ...
    String? host;
    // 本地补丁：响应体一旦开始向客户端推流（headers 已写出），后续上游
    // HttpException 不能再走 HTTP 0.9 兜底——detachSocket 会抛
    // 「Bad state: Headers already sent」（后台挂起冻结推流后恢复即触发）。
    // 声明在 try 外，catch 子句中可见。
    ...
    try {
      ...
      if (uri.scheme == 'http' && !bodyStarted) {
